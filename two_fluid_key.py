...
def solve_mode1(k, cosmo, precision1=1e-8, precision2=1, max_time=-1.0):
    ...
    res = pert.evolve_array(cosmo, max_time, precision1, precision2)
...
s = cosmo.RH_Mpc()                             # converts from Hubble radius to Mpc⁻¹ using the implemented Rh_Mpc NumCosmo method
Pl = ct.physical_constants['Planck length'][0]
p = ct.parsec
ly = ct.light_year
f1 = ( Pl / (p*s * (10**6)) ) * np.sqrt(8.0 * np.pi / 3)
f2 = ( Pl * (p*s * (10**6)) ) * np.sqrt(8.0 * np.pi / 3)
f1 = 1
f2 = 1
...
class mode_1:
  def __init__(self, k, cosmo, precision1=1e-8, precision2=1, max_time=-1):
    self.NcmMatrix = solve_mode1(k, cosmo, precision1, precision2, max_time)
  ...
  def zeta_r(self):
    return f1 * np.array(self.NcmMatrix.get_col(1).dup_array())

  def s_r(self):
    return f2 * np.array(self.NcmMatrix.get_col(2).dup_array())

  def pz_r(self):
    return f2 * np.array(self.NcmMatrix.get_col(3).dup_array())

  def ps_r(self):
    return f1 * np.array(self.NcmMatrix.get_col(4).dup_array())

  def zeta_im(self):
    return f1 * np.array(self.NcmMatrix.get_col(5).dup_array())

  def s_im(self):
    return f2 * np.array(self.NcmMatrix.get_col(6).dup_array())

  def pz_im(self):
    return f2 * np.array(self.NcmMatrix.get_col(7).dup_array())
...
cosmo = Nc.HICosmoQGRW()  
s = cosmo.RH_Mpc()                             # converts from Hubble radius to Mpc⁻¹ using the implemented Rh_Mpc NumCosmo method
Pl = ct.physical_constants['Planck constant'][0]
p = ct.parsec
ly = ct.light_year
f = ( Pl / (p*s * (10**6)) )**2
...
def specs(cosmo, k_interval, E0 = 1):
  ...
  return [ ( (f1**2)/ (2 * (np.pi**2) ) ) * Pk_a1 , ( (f1**2)/ (2 * (np.pi**2)) ) * Pk_a2 ]
...
class spectra:
  ...
  def values1k(self,k):
    mode = mode_1(k,self.model, precision1=1e-8, precision2=1)
  ...
  def values2k(self,k):
    mode = mode_2(k,self.model, precision1=1e-8, precision2=1)
    n = mode.NcmMatrix.col_len() - 1
    values = []
    for j in range(1,9):
      values.append(mode.NcmMatrix.get(n,j))
  ...
  def corr1(self):
    ...
    for k in self.k:
      ...
      zr1 = np.sqrt(f) * values1[0]
      sr1 = np.sqrt(1/f) * values1[1]
      pzr1 = np.sqrt(1/f) * values1[2]
      psr1 = np.sqrt(f) * values1[3]
      zim1 = np.sqrt(f) * values1[4]
      sim1 = np.sqrt(1/f) * values1[5]
      pzim1 = np.sqrt(1/f) * values1[6]
      psim1 = np.sqrt(f) * values1[7]
    ...
    corr = ( (self.k**3) / (2 * (np.pi**2)) )  * np.array( [ np.array(P_zz), np.array(P_sz),  np.array(P_pzz), np.array(P_psz),
  ...
  def corr2(self):
    ...
    for k in self.k:
      ...
      zr2 = np.sqrt(f) * values2[0]
      sr2 = np.sqrt(1/f) * values2[1]
      pzr2 = np.sqrt(1/f) * values2[2]
      psr2 = np.sqrt(f) * values2[3]
      zim2 = np.sqrt(f) * values2[4]
      sim2 = np.sqrt(1/f) * values2[5]
      pzim2 = np.sqrt(1/f) * values2[6]
      psim2 = np.sqrt(f) * values2[7]
...
#    corr =  (self.k**3) * np.array( [ (f/ 2 * (np.pi**2) ) *  np.array(P_zz), np.array(P_sz),  np.array(P_pzz), (f/ 2 * #(np.pi**2) ) *   np.array(P_psz),
#                                                      (f/ 2 * (np.pi**2) )**(-1) *  np.array(P_ss), (f/ 2 * (np.pi**2) )**(-2) *  #np.array(P_pzs),  np.array(P_pss),
#                                                                      (f/ 2 * (np.pi**2) )**(-2) *  np.array(P_pzpz), #np.array(P_pspz),
#                                                                                        (f/ 2 * (np.pi**2) )**(-2) *  #np.array(P_psps)] )
...
class covmatrix:
  def __init__(self,corr_list):
    ...
    # z column
    self.zz = corr_list[0]
    self.sz = np.absolute(corr_list[1])**2 / (corr_list[0] * corr_list[4])
    self.pzz = np.absolute(corr_list[2])**2 / ( corr_list[0] * corr_list[7] )
    self.psz = np.absolute(corr_list[3])**2 / (  corr_list[0] * corr_list[9] )

    # s column
    self.zs = self.sz
    self.ss = corr_list[4]
    self.pzs = np.absolute(corr_list[5])**2 / (corr_list[4] * corr_list[7])
    self.pss = np.absolute(corr_list[6])**2 / (corr_list[4] * corr_list[9])

    # pz column
    self.zpz = self.pzz
    self.spz = self.pzs
    self.pzpz = corr_list[7]
    self.pspz = np.absolute(corr_list[8])**2 / (corr_list[7] * corr_list[9])
